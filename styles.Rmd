...
If you define text appearance such as font size or text color over and over again in your layouts, use `TextAppearance` instead. They are styles, and Android already defined some for you to use and override. This helps tremendously in keeping your view code clean and structured. Moreover, `TextAppearance` allows you to add two styles to any class that extends `TextView`. That means that you will use `TextAppearance` to modify how the text looks on `EditText`, `Button`, etc..

When you define `TextAppearance`s, use any appropriate `TextAppearance` as the parent and define all attributes needed:
...
Once you define a `TextAppearance`, you can use it in any `TextView` with the `android:TextAppearance` attribute:
...
Not as good as true multiple styles, but you can apply **two** styles for one view with `TextAppearance` flexibly.
