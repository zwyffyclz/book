...
* **2010**: First [Nexus](https://en.wikipedia.org/wiki/Google_Nexus) device is released: the [Nexus One](https://en.wikipedia.org/wiki/Nexus_One). These are Google-developed "flagship" devices, intended to show off the capabilities of the platform.
	...
	_Specs:_ 1Ghz Scorpion; 512MB memory; .37" at 480x800 AMOLED capacitive touch. For comparison, the [iPhone 7 Plus](https://en.wikipedia.org/wiki/IPhone_7) (2016) has: 2.34Ghz dual core A10 64bit Fusion; 3GB RAM; 5.5" at 1920x1080 display.
...
- Note that most of the element attributes_ are **namespaced**, e.g. with an `android:` prefix, to avoid any potential conflicts (so we know we're talking about Android's `text` instead of something else).
